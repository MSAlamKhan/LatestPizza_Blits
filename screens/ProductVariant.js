import {
  ...
  const ProductVariant  = ({ route, navigation ,  }) => {
    const {
      isSignin,
      AddCart,
      RemoveCart,
    ...
    console.log(product , "userfejkfn")
    console.log(userDetails.user_id)
    const productItem = product.value;
    // console.log(productItem , "chaneg")
    // console.log(productItem.id  , "moix" )
    const [isLoading, setLoading] = useState(true);
    const [productData, setProductData] = useState(productItem);
    const [modalVisible, setModalVisible] = useState(false);
    const [imageLoading, setImageLoading] = useState(true);
    const [ProductItemList, setProductItem] = useState([]);
    const [flexDirection, setflexDirection] = useState("column");
    console.log(ProductItemList.Data)
    ...
    const recentlyViewApi = async recent => {
      ...
      try {
        let base_url = `${APIURL}/API/new_get_variations.php`;
        let form = new FormData();
        form.append(
          'token',
          'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC',
        );
        form.append('product_id', "99" );
    ...
    // const [productData, setProductData] = useState(productItem);
    const [addons, setaddons] = useState({items: [productData?.addons[0]?.ao_data],addonstotalAmount: 0,
    });
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={[styles.header]}>
          <Image
            ...
            resizeMode="cover"
            source={{
              uri: `${APIURL}/admin_panel/Uploads/${product.value?.img}`,
            }}
          />
          <ActivityIndicator
        ...
        <View style={styles.lowerlayout}>
          ...
          />
          <View>
            <Text style={{marginLeft:18,  }} >{ProductItemList.sub_title}</Text>
          </View>
    
          {/* <PreviewLayout
        label={ProductItemList.Data ? ProductItemList?.Data[0].sub_title : null}
       ...
       <View style={{ padding: 10 , flexDirection:"row" , flexWrap:"wrap"}}>
        <View style={styles.row}>
          ...
          {ProductItemList.Data ? <>
            {ProductItemList.Data.map((value) => (
      ...
      <View style={{ padding: 10 , flexDirection:"row" , flexWrap:"wrap"}}>
        ...
        <View style={styles.row}>
          ...
          {ProductItemList.Data ? <>
            {ProductItemList.Data.map((value , index ) => (
              <>
            <TouchableOpacity
              key={value}
              // onPress={() => setSelectedValue(value)}
            ...
            // >
               style={[
                  styles.button13,
                  // value.product_id === index && styles.selected,
              ]}>
              <Text
                style={
                  styles.buttonLabel}
